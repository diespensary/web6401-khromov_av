...
function isInteger(n) {
  ...
  // Если n не число, вернем false
  if (typeof n !== 'number') return false;
  // Побитовое ИЛИ с 0 не изменяет целые числа, но отбрасывает дробную часть
  return (n | 0) === n;
}
...
function isBinary(n) {
  if (n <= 0) return false;
  // Число является степенью двойки, если в двоичном представлении
  // у него только одна единица
  return (n & (n - 1)) === 0;
}
...
function deepEqual(firstObject, secondObject) {
  ...
  // Проверка на NaN (NaN !== NaN, но мы считаем их равными)
  ...
  // Если оба являются функциями, сравниваем их код
